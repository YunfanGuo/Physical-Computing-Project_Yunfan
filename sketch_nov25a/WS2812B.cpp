...
WS2812B::WS2812B() :
  begun(false), pixels(NULL),pixels1(NULL), endTime(0)
{
#if defined(NRF5)
  strip->leds = NULL;
  strip1->leds = NULL;
...
#if defined(NRF5)
  if(strip->leds)   {free(strip->leds); strip->leds = NULL;}
  if(strip1->leds)  {free(strip1->leds); strip1->leds = NULL;}
#endif
  if(pin >= 0) pinMode(pin, INPUT);
}

void WS2812B::begin(uint8_t p, uint16_t n, uint8_t bright, neoPixelType t) {
  if(pin != -1){
...
void WS2812B::updateLength(uint16_t n) {
  if(pixels) {free(pixels); pixels = NULL;}// Free existing data (if any)
  if(pixels1) {free(pixels1); pixels1 = NULL;}// Free existing data (if any)
...
void WS2812B::updateType(neoPixelType t) {
...
void WS2812B::rotate(int8_t offset) {

    if (abs(offset) <= 0) return;
#if defined(NRF5)
    if (strip->num_leds <= 0) return;
...
void WS2812B::showBarGraph(uint16_t start, uint16_t end, int16_t value, int16_t high) {
    if(end < start) {
        uint16_t num = end;
        end = start;
        start = num;
    ...
    uint16_t n = end - start + 1;
            ...
            if (v == 0) {
                ...
                for (uint16_t i = 0; i < n; ++i) {
                    ...
                    else this->setPixelColor(i + start, 0, false);
                }
            }
            this->show();
...
void WS2812B::shift(int8_t offset) {
...
void WS2812B::showRainbow(uint16_t start, uint16_t end, uint32_t startHue, uint32_t endHue) {
...
uint32_t WS2812B::hsl(uint32_t h, uint32_t s, uint32_t l) {
    h = h % 360;
	s = max(s, 0);
	s = min(s, 99);
	l = max(l, 0);
...
void WS2812B::setRangeColor(int16_t start, int16_t end, uint32_t c) {
...
void WS2812B::setPixelColor(uint16_t n, uint32_t c, bool user) {
...
uint8_t *WS2812B::getPixels(void) const {
  return pixels;
}
...
void WS2812B::setBrightness(uint32_t b) {
...
uint32_t WS2812B::rgbToColor(uint8_t r, uint8_t g, uint8_t b)
{
	uint32_t cr = r;
	uint32_t cg = g;
	uint32_t cb = b;
